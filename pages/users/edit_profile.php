...
if (!$user_id) {
    echo "<div class='bg-red-50 border border-red-200 p-4 mb-6 rounded-lg shadow-sm flex items-center' role='alert'>
            ...
            <span class='text-red-700 font-medium'>You must be logged in to edit your profile!</span>
          </div>";
    exit;
}
...
// Process form submission for updating the user profile
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE users 
            SET username = ?, full_name = ?, email = ?, contact = ?
            WHERE id = ?";
    ...
    if ($stmt->execute()) {
        echo "<div class='bg-green-50 border border-green-200 p-4 mb-6 rounded-lg shadow-sm flex items-center' role='alert'>
                ...
                <span class='text-green-700 font-medium'>Profile updated successfully!</span>
              </div>";
    } else {
        echo "<div class='bg-red-50 border border-red-200 p-4 mb-6 rounded-lg shadow-sm flex items-center' role='alert'>
                <i class='bx bx-error-circle text-xl text-red-500 mr-2'></i>
                <span class='text-red-700 font-medium'>Error updating profile: " . $stmt->error . "</span>
              </div>";
    }
}
?>
